            ...
            <div class="card">
                <div class="card-header py-2">
                    <h4 class="mb-0 cd-title d-flex align-items-center justify-content-between">{{ $title }}
                        ...
                        <a href="{{ route('admin.testimonials.create') }}" class="btn btn-sm btn-primary rounded-0"><i class="fas fa-plus fa-sm"></i> Add Testimonial</a>
                        @endpermission
                    </h4>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-sm table-hover table-striped" id="testimonial-datatable">
                            <thead>
                                <th>SL</th>
                                <th>Image</th>
                                <th>Full Name</th>
                                ...
                                <th>Created By</th>
                                <th>Created At</th>
                                @if (permission('testimonial-edit') || permission('testimonial-delete'))
...
@push('scripts')
    <script>
        table = $('#testimonial-datatable').DataTable({
            processing: true,
            serverSide: true,
            responsive: false,
        ...
        $(document).on('click', '.delete_data', function () {
            ...
            let url = "{{ route('admin.testimonials.delete') }}";
            delete_data(id,url,row,name);
        });
        @endpermission

        @permission('testimonial-status')
        // status changes
        $(document).on('click', '.change_status', function() {
            var id = $(this).data('id');
            var name = $(this).data('name');
            var status = $(this).data('status');
            var url = "{{ route('admin.testimonials.status-change') }}"
