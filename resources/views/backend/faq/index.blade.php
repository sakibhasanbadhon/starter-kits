            ...
            <div class="card">
                <div class="card-header py-2">
                    <h4 class="mb-0 cd-title d-flex align-items-center justify-content-between">{{ $title }}
                        ...
                        <a href="{{ route('admin.faqs.create') }}" class="btn btn-sm btn-primary rounded-0"><i class="fas fa-plus fa-sm"></i> Add FAQ</a>
                        @endpermission
                    </h4>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-sm table-hover table-striped" id="faq-datatable">
                            <thead>
                                <th>SL</th>
                                <th>Question</th>
                                <th>Answer</th>
                                ...
                                <th>Created By</th>
                                <th>Created At</th>
                                @if (permission('faq-delete') || permission('faq-edit'))
...
@push('scripts')
    <script>
        ...
        @permission('faq-status')
        // status changes
        $(document).on('click', '.change_status', function() {
            var id = $(this).data('id');
            var name = $(this).data('name');
            var status = $(this).data('status');
            var url = "{{ route('admin.faqs.status-change') }}"
