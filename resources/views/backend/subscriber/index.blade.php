            ...
            <div class="card">
                <div class="card-header py-2">
                    <h4 class="mb-0 cd-title d-flex align-items-center justify-content-between">
                        {{ $title }}
                        @permission('subscriber-broadcast')
                        <a href="#" class="btn btn-sm btn-success shadow-none"><i class="fa fa-reply fa-sm"></i> Send Mail</a>
                        @endpermission
                    </h4>
                </div>
                <div class="card-body">
                    <div class="table-responsive">
                        <table class="table table-bordered table-sm table-hover table-striped" id="subscriber-datatable">
                            <thead>
                                <th>SL</th>
                                <th>Email</th>
                                <th>Created At</th>
...
@push('scripts')
    <script>
        table = $('#subscriber-datatable').DataTable({
            processing: true,
            serverSide: true,
            responsive: false,
        ...
        $(document).on('click', '.delete_data', function () {
            ...
            let url = "{{ route('admin.subscribers.delete') }}";
